...
with st.expander("1️⃣ 기본 정보 & 자기 인식 (Baseline)", expanded=True):
    ...
    with col1_1:
        solo_duration = st.selectbox("현재 연애 상태 (솔로 기간)", ["6개월 미만", "6개월~2년", "2년 이상", "모태솔로"], key="solo_duration")
        # *** 성별 선택 (매우 중요!) ***
...
if st.button("🔮 시뮬레이션 실행!"):
    ...
    with col_res1: st.metric("🌟 만남 확률 (6개월)", f"{results_df.loc['6개월', '만남 확률 (%)']:.1f}%")
    with col_res2: st.metric("💖 연애 시작 확률 (6개월)", f"{relationship_prob_6m:.1f}%")
    ...
    share_text_insta = f"""
    💖 내 6개월 연애 확률: {relationship_prob_6m:.1f}% 💖
    ({char_feeling_text})
    ...
    **모든 핵심 원리와 설계 비밀**은 책 속에 담겨 있습니다.
    """)
    # !!! 실제 책 구매 링크로 꼭 변경하세요 !!!
    book_purchase_link = "https://search.yes24.com/search?domain=STORE&query=%EC%8B%9C%EB%AE%AC%EB%A0%88%EC%9D%B4%EC%85%98%EB%90%9C%20%EB%B2%A0%EC%8A%A4%ED%8A%B8%EC%85%80%EB%9F%AC" # 예시: YES24 검색 링크
    st.link_button("👉 『시뮬레이션된 베스트셀러』 구매하고 설계자 되기!", book_purchase_link, type="primary")
    ...
    st.text_area(
        ...
        "중요한 건 데이터로 사고하는 '연습'을 통해, 내 삶의 변수를 직접 설계해보려는 '의지'입니다. 자, 이제 어떤 변수를 바꿔서 그(녀)의 더 환한 미소를 보시겠어요?",
        height=250, # 높이 약간 늘림
...
# --- 버튼 클릭 전 초기 화면 안내 ---
    ...
    book_purchase_link_init = "https://search.yes24.com/search?domain=STORE&query=%EC%8B%9C%EB%AE%AC%EB%A0%88%EC%9D%B4%EC%85%98%EB%90%9C%20%EB%B2%A0%EC%8A%A4%ED%8A%B8%EC%85%80%EB%9F%AC"
    st.link_button("『시뮬레이션된 베스트셀러』 미리보기", book_purchase_link_init)
